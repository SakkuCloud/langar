...
func main() {
	...
	var configFile = flag.String("c", "/etc/langar/config.yml", "config file location (must be yaml format)")
	flag.Parse()
	...
	// config file
	if *debugMode {
		err := configor.New(&configor.Config{ENVPrefix: "LANGAR", Verbose: true}).Load(&config.Config, *configFile)
		if err != nil {
			log.Fatal(err)
		}
	} else {
		err := configor.New(&configor.Config{ENVPrefix: "LANGAR"}).Load(&config.Config, *configFile)
		if err != nil {
			log.Fatal(err)
		}
	}
